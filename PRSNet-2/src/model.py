...
class AttentiveReadout(nn.Module):
    def __init__(self, in_feats):
        ...
        self.weight_layer = nn.Sequential(
            nn.Linear(in_feats, 1, bias=False), nn.Sigmoid()
        )
...
class SNP2Gene(nn.Module):
    ...
    def forward(self, snp):
        ...
        snp_h_list = [
            torch.einsum(
                "bnd,n->bnd",
                self.adaptive_dropout(snp.to(device), self.pvalues),
                self.filter_list[i].to(device),
            )
            for i in range(self.n_snp_kernels)
        ]
        ...
        gene_features_reduced = torch.segment_reduce(
            data=data_reshaped, reduce="sum", lengths=self.gene2snp_len, axis=0
        )
        gene_h = gene_features_reduced.reshape(
            -1, batch_size, self.n_snp_kernels
        ).permute(
            1, 0, 2
        )  # .reshape(-1, self.n_snp_kernels)
        gene_h = torch.einsum(
            "bng,ngf->bnf",
            gene_h,
            self.gene_proj.weight.view(self.n_genes, self.n_snp_kernels, self.d_gene),
        )
    ...
    def adaptive_dropout(self, snp, p_value):
        if self.training:
            ...
            random_mask = (
                torch.rand(snp.shape[0], snp.shape[1]).to(snp.device) < p_value
            )
...
class PRSNet2(nn.Module):
    def __init__(
        ...
        self.gene_encoder = SNP2Gene(
            ...
            pvalues,
            d_gene=d_hidden,
            n_snp_kernels=n_snp_kernels,
            sg_dropout_init=sg_dropout_init,
            sg_dropout_min=sg_dropout_min,
        )
        ## GIN
