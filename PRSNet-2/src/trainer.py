...
class Trainer:
    def __init__(self, args, model, g, pvalues, device, sg_l1, lr=1e-4, weight_decay=0, eval_interval=100, n_steps=50000, n_early_stop=100, log_interval=20, model_name='model'):
        self.args = args
        self.model = model
    ...
    def train(self, train_loader, val_loader, test_loader):
        ...
        for i in range(len(train_loader)):
            ...
            l1_loss = torch.sum(torch.stack([torch.sum(torch.abs(param) * self.pvalues) for param in self.model.gene_encoder.filter_list]))/len(self.model.gene_encoder.filter_list)/torch.sum(self.pvalues)
            ...
            if (i+1) % self.log_interval == 0:
                print(f"[{i+1}] pred loss: {np.mean(running_pred_loss):.3f}", flush=True)
            ...
            if (i+1) % self.eval_interval == 0:
                ...
                print(f"[{i+1}] val_auroc: {val_auroc_score:.5f}, val_ap: {val_ap_score:.5f}, test_auroc: {test_auroc_score: .5f}, test_ap: {test_ap_score: .5}")
                print("----------------Training----------------")
                self.model.train()
    ...
    def evaluate(self, test_loader):
        with torch.no_grad():
            ...
            return self.auroc(preds, labels).item(),self.ap(preds, labels.long()).item()
    def predict(self, data_loader):
        with torch.no_grad():
            ...
            for inputs, labels in data_loader:
                ...
                weights_list+=weights.detach().cpu().numpy().reshape(len(labels),-1).tolist()
